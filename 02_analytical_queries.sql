...
-- Three advanced analytical queries built on CTEs + window functions.
-- Target: PostgreSQL 13+
-- Tested logic equivalent validated in DuckDB against the sample data.
-- ============================================================


-- ============================================================
-- QUERY 1: ABC (Pareto) Product Classification
-- Ranks products by revenue contribution and buckets them into
-- A (top 80% of revenue), B (next 15%), C (bottom 5%).
-- Business use: focus forecasting/replenishment effort on "A" items.
-- ============================================================
...
-- Uses a trailing 30-day window to compute each product's average
-- daily sales rate, then estimates how many days of stock remain
-- and flags products that will run out before their next restock
-- (lead time) arrives.
-- Business use: this is your daily "what needs a PO today" list.
-- ============================================================
WITH daily_sales AS (
...
recent_velocity AS (
    SELECT
        product_id,
        AVG(units_sold) AS avg_daily_velocity_30d,
        STDDEV_SAMP(units_sold) AS stddev_daily_velocity_30d
    FROM daily_sales
    WHERE date >= (SELECT MAX(date) FROM daily_sales) - INTERVAL '30 days'
    GROUP BY product_id
),
latest_stock AS (
    SELECT
...
-- Classic supply-chain formula:
--   Safety Stock   = Z * stddev(daily demand) * sqrt(lead time)
--   Reorder Point  = (avg daily demand * lead time) + Safety Stock
-- Z = 1.65 corresponds to a ~95% service level (95% chance of NOT
-- stocking out during the lead-time window). Raise Z for a higher
-- service level (e.g. 2.33 for ~99%), at the cost of more inventory.
-- Business use: replaces the arbitrary reorder_point in the
-- products table with a statistically justified one.
-- ============================================================
WITH daily_sales AS (
    SELECT product_id, date, SUM(units_sold) AS units_sold
    FROM sales_transactions
...
demand_stats AS (
    SELECT
        ...
        STDDEV_SAMP(units_sold) AS stddev_daily_demand
    FROM daily_sales
    GROUP BY product_id
)
SELECT
